...
/* eslint-disable -- generated assistant-ui registry component */
...
function ImageFilename({ className, children, ...props }: React.ComponentProps<"span">) {
  ...
  return (
    <span
      ...
      className={cn("block truncate px-2 py-1.5 text-xs text-muted-foreground", className)}
      {...props}
    >
      {children}
    </span>
      ...
      {isMounted &&
        ...
        createPortal(
          <div
            ...
            role="dialog"
            aria-modal="true"
            className="aui-image-zoom-overlay fixed inset-0 z-50 flex animate-in items-center justify-center bg-black/80 duration-200 fade-in"
            onClick={handleClose}
          ...
          >
            <img
              ...
              src={src}
              alt={alt}
              className="aui-image-zoom-content max-h-[90vh] max-w-[90vw] animate-in cursor-zoom-out object-contain duration-200 zoom-in-95 fade-in"
              onClick={(e) => {
                e.stopPropagation();
                handleClose();
              }}
            ...
            <button
              ...
              className="absolute end-4 top-4 cursor-pointer rounded-md bg-background/80 p-2 text-muted-foreground hover:text-foreground"
            >
...
function ImageGenerating({ className }: { className?: string }) {
  return (
    <div
      ...
      className={cn("flex min-h-32 items-center justify-center bg-muted/50 p-4", className)}
    >
