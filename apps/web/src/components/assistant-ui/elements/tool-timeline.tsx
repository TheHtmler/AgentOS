        ...
        <div className="flex flex-col gap-2.5 ps-4 pt-2.5">
          {take(steps, visibleSteps).map((step, index, shown) => {
            ...
            return (
              <div
                key={step.chip}
                className="flex animate-in items-center gap-2 text-[13.5px] text-foreground/55 duration-300 fill-mode-both fade-in slide-in-from-bottom-1"
              >
                ...
                <span className="rounded-md bg-foreground/[0.06] px-1.5 py-0.5 font-mono text-[11px] text-foreground/70">
                  {step.chip}
                </span>
              </div>
            );
