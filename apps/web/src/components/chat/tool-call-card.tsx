...
export function SandboxFilePreviewPane({
  ...
  return (
    ...
    >
      ...
      <div className="min-h-0 flex-1 [scrollbar-width:thin] overflow-auto bg-muted/20 p-3">
        {textFile ? (
      ...
      {toolCall.expanded ? (
        <div className="grid gap-2 border-t border-border bg-muted/20 px-3 py-2.5 text-xs text-foreground/90">
          ...
          {knowledgeHits.length > 0 ? (
            <div>
              <span className="mb-1 inline-block font-semibold text-muted-foreground">
                命中片段
              </span>
          ...
          {sandboxCommand !== null ? (
            <div className="grid gap-1.5 rounded-md border border-border bg-card/60 p-2">
              ...
              {sandboxStdout ? (
                <pre className="max-h-40 [scrollbar-width:thin] overflow-y-auto rounded-md bg-muted/40 p-2 font-mono text-[0.7rem] whitespace-pre-wrap text-foreground/90">
                  {sandboxStdout}
                </pre>
              ) : null}
              {sandboxStderr ? (
                <pre className="max-h-40 [scrollbar-width:thin] overflow-y-auto rounded-md bg-destructive/5 p-2 font-mono text-[0.7rem] whitespace-pre-wrap text-destructive">
                  {sandboxStderr}
                </pre>
              ) : null}
          ...
          {artifactText !== null ? (
            <div>
              ...
              <pre className="mt-1 max-h-40 [scrollbar-width:thin] overflow-y-auto rounded-md bg-muted/50 p-2 font-mono text-[0.7rem] whitespace-pre-wrap">
                {artifactText}
              </pre>
            </div>
          ) : null}
