...
## 2. 可调用的 MCP（外部调研，尚未接入 AgentOS）

当前 Cursor 会话里**没有**挂载医学类 MCP；下列为可评估接入 Agent Runtime 的候选。
...
在 MCP 未接入前，P0 prompt 已要求：**缺公开标准时先 search/fetch，禁止让用户代查**。
...
- **当前会话 MCP**：仅有 Cursor 控制与 mcp-ui 文档，**未挂载**任何医学 MCP。  
