...
**Ops 管理台第一期（2026-08-13，已落地）：** 侧栏壳子与概览；知识详情/元数据/chunks/快照预览；Agent 启停与默认。

**Ops 管理台第二期（2026-08-16，已落地）：** 会话只读审计、智能体不可变发版、知识快照恢复、统一工具目录（内建/MCP 来源视图）。多账号、chunk 编辑、外部 MCP 服务器配置仍属后续增量。
