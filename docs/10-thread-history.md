...
当前助手 Message 仅在 Run `completed` 时写入。AG-UI 的浏览器 SSE 断开后，Agent API 进程内的 Run 会继续执行并持久化最终结果；用户回到前台后，页面等待 Run 进入终态，再刷新该 Thread 的历史，不拼接不完整的 `text_delta`。显式点击停止时才会调用取消接口。服务进程重启时，启动流程会从仍为 `running/queued` 的 Run 和最后一条 user Message 重建 AG-UI 请求并重新排入当前进程；若所有者已停用或缺少有效 prompt，则保留原 Run 供 Ops 诊断。若该 Run 已有终态 `tool_result`，仍可显示完成态工具卡。
