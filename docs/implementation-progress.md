...
- Ops 知识库多途径导入：JSON / 纯文本 / 链接 / 文件（`.txt` / `.md` / `.json`）/ PDF / 图片（`.jpg` `.png` `.webp`，OCR 后切块）；统一 ingest → 同 slug 快照覆盖 upsert；PDF 文本层不足或图片导入时调用本机 PaddleOCR HTTP（`OCR_BASE_URL`）；Ops UI 导入页 + BFF `POST /api/ops/knowledge/import`。图片会先转成 OCR 友好 PNG；适配层递归解析 PaddleX `prunedResult.rec_texts` 等包装；`/ocr/file` 空结果会回退 `/ocr`。
