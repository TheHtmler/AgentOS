...
Web 聊天执行流按 DSH Web 的信息层级收敛：每轮步骤以左侧锚定的可折叠执行组承载，工具调用和 Thinking 均压缩为单行摘要（工具或阶段、关键参数或结果、状态、耗时），完整内容、结构化结果、文件和错误详情按需展开并限制自身滚动高度；`sandbox_exec` 的命令、退出码、stdout/stderr 与生成文件以终端式专属视图呈现。现有 AG-UI、run_events 历史回放、HITL 和 sandbox 文件预览接口保持不变。
