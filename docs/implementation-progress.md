...
- Ops 知识库多途径导入：JSON / 纯文本 / 链接 / 文件（`.txt` / `.md` / `.json`）/ PDF / 图片（`.jpg` `.png` `.webp`）；统一 ingest → 同 slug 快照覆盖 upsert；PDF/图片导入逐页/逐图调用远程 `BACKGROUND_VISION_MODEL`（`knowledge/vision_extract.py`，未配置直接 400，不回落本地 OCR）转录文字并描述图表，PDF 单页失败回退该页文本层；Ops UI 导入页 + BFF `POST /api/ops/knowledge/import`。
