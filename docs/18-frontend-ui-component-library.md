...
- `components/files/file-library-panel.tsx` 使用 shadcn `Button` / `Input` / `Dialog` / `AlertDialog` 与 lucide 图标，实现上传文件与系统生成文件的缩略卡片网格（默认 24 项，按需加载更多）、图片/PDF 首页缩略图、搜索、图片/PDF 预览、下载、来源会话跳转和删除确认。
