...
- Ops 知识库 PDF/图片导入不走本地 PaddleOCR：`knowledge/vision_extract.py` 用同一个已认证的后台端点逐页/逐图调用 `BACKGROUND_VISION_MODEL`（无回落 `BACKGROUND_CHAT_MODEL`——通常是纯文本模型，配错会直接调用失败），按真实阅读顺序转录文字并对图表/示意图给出文字描述，而不是像文本层优先方案那样对文字够多的页面整页跳过嵌入的图；PDF 单页视觉调用失败时回退该页 PyMuPDF 文本层，仍失败则跳过该页，不拖垮整份文档；`BACKGROUND_VISION_MODEL` 未配置时导入直接 400，不会静默退回旧的本地 OCR。本地 PaddleOCR sidecar（`:8787`）继续只服务聊天内文档上传抽取（`uploads/extract.py`），未被取代。
