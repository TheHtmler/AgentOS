...
- 语音输入 → 浏览器按住 `MediaRecorder` → Web BFF → `POST /v1/audio/transcriptions` → 显式配置的 ASR。`ASR_PROVIDER=openai_compatible` 调 `/audio/transcriptions`；本地 `whisper_cpp` 调其 loopback `/inference`（服务用 ffmpeg 转 WebM，`ASR_LANGUAGE` 默认 `zh`）。ASR 默认关闭；音频仅在请求内存中转发、不写 Artifact/消息历史，`[Music]`/静音等无效转写拒绝回填，转写文本由用户确认发送，不换聊天模型。
