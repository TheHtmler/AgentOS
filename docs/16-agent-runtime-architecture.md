...
- 语音输入 → 浏览器 `MediaRecorder` → Web BFF → `POST /v1/audio/transcriptions` → 显式配置的 ASR。`ASR_PROVIDER=openai_compatible` 调 `/audio/transcriptions`；本地 `whisper_cpp` 调其 loopback `/inference`（服务用 ffmpeg 转 WebM）。ASR 默认关闭；音频仅在请求内存中转发、不写 Artifact/消息历史，转写文本回填 composer 后由用户确认发送，不换聊天模型。
