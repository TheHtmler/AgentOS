...
- 语音输入 → 已落地为浏览器 `MediaRecorder` → Web BFF → `POST /v1/audio/transcriptions` → 显式配置的 OpenAI-compatible ASR sidecar/gateway（如 whisper.cpp / FunASR）。ASR 默认关闭；音频仅在请求内存中转发、不写 Artifact/消息历史，转写文本回填 composer 后由用户确认发送，不换聊天模型。
