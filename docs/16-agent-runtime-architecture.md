...
- 后台任务（自动标题 / 记忆 / Case 抽取）和 embedding 固定走显式 `BACKGROUND_*` 端点，不随 Agent Provider 变化；凭据不入库、不进响应。知识库向量按模型、维度和 `context-v2` 检索文本版本校验，未知/过期向量不用。可从 Ops 文档详情异步重建向量，无需重传原件；模型切换必须同步 `KNOWLEDGE_EMBEDDING_DIMENSIONS`。详见 [20-knowledge-retrieval.md](20-knowledge-retrieval.md)。
- Ops 知识库导入为异步任务，阶段为 extracting → embedding → persisting → complete；失败记录阶段、页级质量与解析检查点。原件按 SHA-256 私有归档，支持 Ops 下载、重新解析和失败重试。PDF/图片调用 `BACKGROUND_VISION_MODEL`，可通过 `BACKGROUND_VISION_BASE_URL/API_KEY` 使用独立端点；视觉超时默认 180 秒，与聊天 OCR 分离。视觉输出截断受控重试，缺失页禁止发布，文本层回退进入待复核；成功页缓存用于重试。PDF 只渲染活跃页，避免保留整本位图。批量 embedding 在事务外完成，短事务替换内容；退化新版保留可用旧版。同文档锁覆盖提交、修改、删除与快照恢复。原件/任务质量/存量迁移详见 docs/20；本地 PaddleOCR 仍只用于聊天附件。
- 证据编审（Ops-only）：Ops 可通过固定的 PubMed E-utilities 端点检索最多 10 条候选，并按 PMID 导入**摘要**。候选文档强制标为 `pending_review` 和 `research_article`，运行时 `knowledge_search` 只读取 `curated` / `clinically_reviewed`，因此任何新论文不会未经人工核对直接进入家庭对话。文档可保存术语的 `curie` / 标签 / ontology 元数据；Ops 可通过 OLS 查询 MONDO、HPO 或 Orphanet 候选后明确选择，导入时 CURIE 追加到每个 chunk 的 tags，用于疾病、亚型和表型的确定性召回。该 Ops 流程不接触 Case、上传件或患者查询，也不把第三方 Skill 作为运行时插件安装。
- 向量落库可观测性：Ops 文档列表与详情只统计当前模型、维度、检索文本版本匹配的有效非零数值数组；JSON null 不算有效向量。导入正文完成与向量完成分开呈现。快照恢复在事务外重新向量化，落库前比较文档版本；并发修改时返回 409。
...
- 当知识库或用户附件已有明确的“条件 → mg/kg 剂量/次数”表时，Agent 需要先核对命中条件，再用 `calculate` 对已知体重做条件式换算；缺少决定性指标时必须点出条件，不能将“血氨正常”误作“血精氨酸已知”，也不能用泛泛的自行用药提示替代既有表格的数值核对。结果是既有表/处方的换算，不是新增医嘱。
- Case 标准槽位的同值重复（如 `15.2 kg` 与 `体重 15.2 kg`）按字段语义幂等：不新建、不刷新 `recorded_at`、不再次触发确认。后台抽取的变化值只能写为 `proposed`，不得覆盖已确认的 Current；经对应 HITL 确认后才替换 Current。
- 月龄的明确近似复述（如已确认 `18.1 个月`，候选 `约 18 月龄`）在候选所述精度内去重，保留 Current 内容与时间；不把这类候选再次列入更新提醒。无近似标记的不同精确值，以及身高、体重变化不应用此容差。`case/values.py` 的相同比对同时用于写入、待确认 API 与模型 Proposed 上下文；旧的同义待确认行保留审计记录，但不再作为待更新项展示。口语月龄提取保留小数与近似标记，避免把 `18.1 个月` 截成 `1 个月`。真正的档案写入仍必须经过原有 HITL。
- 查询侧：口语、常用中英文医学术语扩展优先保留，中文二元词补充匹配；疾病标签只对已有相关性的关键词结果加权。全部授权候选按 128 条流式读取，各路保留相关性最高的有限候选，再用 RRF 融合去重，禁止按文档名截取前 200 条。同章节相邻片段单独携带来源 ID。工具事件记录扫描数、有效文档向量数、降级原因及各路排名；`/v1/ops/knowledge/search` 使用已发布 Agent 的知识范围提供同链路调试。
...
- **文件库（P0）**：产品端左侧「文件库」经 `GET /v1/me/files` 只列出当前用户有持久化原件的 `kind="upload"` 或 `kind="sandbox"` Artifact，提供搜索、图片缩略图、PDF 首页缩略图、图片/PDF 预览、下载、回到来源 Thread 和删除。`sandbox_exec` 发现新增或修改的文件后，会用当前用户身份从私有 Manager 拉取受限字节，复制到 owner-scoped `UPLOAD_ROOT`，再创建 `kind="sandbox"` 条目；工作区路径只作为来源元数据，文件库不依赖其长期存在。它刻意不暴露 `fetch_url` 正文、Sandbox stdout 等模型工作数据；账户弹窗只管理长期记忆，文件删除统一从文件库进入，并继续通过 owner 过滤及原件删除执行。当前聊天上传类型仍为 PDF/PNG/JPEG/WebP；CSV/XLSX/DOCX/PPTX 的安全转换预览，以及 Ops 管理的公共资料版本库，均是后续阶段，不能把 `Artifact` 通用表误认为已经具备这两种产品能力。
...
- 交互护栏：Memory embedding 在交互路径有独立的 `MEMORY_RECALL_TIMEOUT_SECONDS`（默认 2 秒），不会继承后台抽取的 30 秒超时；普通模型流和 HITL resume 受 `INTERACTIVE_RUN_TIMEOUT_SECONDS`（默认 10 分钟）限制，超时会把 Run 标记为失败并释放前端状态。
...
- `model_step` 事件还记录 `preflight_ms` 与 `queue_wait_ms`，分别拆分上下文准备和 Provider semaphore 排队耗时；Web 会话统计展示最近一轮的准备/排队时间，避免把所有首 token 延迟都归因于模型。
- Responses 交互请求显式使用 `INTERACTIVE_REASONING_EFFORT`（默认 `low`），不再依赖 GPT-5.6 Provider 的默认 reasoning effort；需要更高质量推理的 Agent 可在后续增加按 AgentVersion 的覆盖项。
- 观测加深 →（已落地）`run_events` 新增 `tool_result.duration_ms`（六个工具模块统一打点）、`model_step`（整个 run 的墙钟耗时 + token 用量，`api/chat.py::persist_model_step_event`, best-effort 不影响主流程）与 `context_budget`（pre_run/step 裁剪事实，`persist_context_budget_event`, fire-and-forget，同为 best-effort）；Ops `GET /v1/ops/sessions/{thread_id}/runs/{run_id}/events` + 会话详情页「查看事件」可读时间线。另有可选 Langfuse/OTel 旁路：`observability.py` 在启用并配置 key 后调用 PydanticAI instrumentation，AG-UI 普通 run 与 HITL resume 建立 `thread_id` → session、`run_id` → trace 的关联，并默认关闭 prompt/output、二进制、附件、知识库原文和 reasoning 采集；Langfuse 不参与 run 状态机或持久化，发送失败不影响主流程。自建 Langfuse 不部署在 16GB Mac mini。`model_step` 另带 `ttft_ms`（首个文本/推理内容到达延迟）与 `cached_input_tokens`（`usage.cache_read_tokens`, DeepSeek 取 `details.prompt_cache_hit_tokens`; 未报告缓存命中时为 null）；产品聊天页 composer 下方的会话状态栏经 `GET /v1/threads/{id}/stats` 展示轮数/步数/LLM 与工具耗时/首 token 平均/token 总量与上下文进度（最近一轮实际 `input_tokens` 对 provider `context_window`），流式中切换为本轮耗时/工具次数/首 token/约 tok/s。
