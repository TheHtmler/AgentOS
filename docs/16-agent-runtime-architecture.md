...
- **文件库（P0）**：产品端左侧「文件库」经 `GET /v1/me/files` 只列出当前用户 `kind="upload"` 且有持久化原件的 Artifact，提供搜索、图片/PDF 预览、下载、回到来源 Thread 和删除。它刻意不暴露 `fetch_url` 正文、Sandbox stdout 等模型工作数据；Sandbox 新增文件仍只在其工具结果位置通过 Manager 受控代理读取，尚未复制成可长期管理的文件库条目。账户弹窗只管理长期记忆，文件删除统一从文件库进入，并继续通过 owner 过滤及上传原件删除执行。当前聊天上传类型仍为 PDF/PNG/JPEG/WebP；CSV/XLSX/DOCX/PPTX 的安全转换预览，以及 Ops 管理的公共资料版本库，均是后续阶段，不能把 `Artifact` 通用表误认为已经具备这两种产品能力。
